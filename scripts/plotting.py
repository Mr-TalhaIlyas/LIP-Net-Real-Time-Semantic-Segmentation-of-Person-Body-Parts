...
def plot_results(path, im_height, im_width, model, show_img = 5, clr_map='gray', Threshold = 0.5, 
                 pallet=pallet_ADE20K, R_seed = 5, gray2rgb=False, clr_masks = True, activation=False):
...
#%%
'''
img = cv2.imread(path_train+'/images/images/img_86.jpg')
...
#                    For only images
#**************************************************************
img_dir = '/home/user01/data_ssd/Talha/vistas_edge/val/images/images'
...
# For resizing...
# 1. Use interpolation=cv2.INTER_NEAREST for resizing masks.
# 2. Use interpolation=cv2.INTER_LINEAR  for resizing images.
save_npy = True
...
for filepath in tqdm(img_paths):
    ...
    if save_npy == False:
        z = (z > 0.5).astype(np.uint8)
        z=z[0,:,:,0]
        cv2.imwrite('/home/user01/data_ssd/Talha/preds_seed/val/img_{}.jpg'.format(i+1),z*255)
    if save_npy == True:
        z = z.squeeze()
        np.save('/home/user01/data_ssd/Talha/preds_seed/val/img_{}.npy'.format(i+1),z)
    i = i+1
'''